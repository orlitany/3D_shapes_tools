function [shape] = sphere_tri(shape,maxlevel,r,winding)
%        [shape] = sphere_tri('ico',4,0.5,0);
...
warning off
shape.TRIV = FV.faces;
% shape.X = FV.vertices(:,1)+r;
% shape.Y = FV.vertices(:,2)+r;
% shape.Z = FV.vertices(:,3)+r;

shape.X = FV.vertices(:,1)+0;
shape.Y = FV.vertices(:,2)+0;
shape.Z = FV.vertices(:,3)+0;

xyz = [shape.X, shape.Y, shape.Z];

% shape.C = xyz;
shape.vec=1*ones(size(shape.X));
